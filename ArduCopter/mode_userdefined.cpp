...
bool ModeUserDefined::init(bool ignore_checks)
{
    ...
    copter.user_nav->init();

#if HAL_MOUNT_ENABLED
    // Check if the CIRCLE_OPTIONS parameter have roi_at_center
    if (copter.user_nav->roi_at_center())
    {
        const Vector3p &pos{copter.user_nav->get_center()};
...
void ModeUserDefined::run()
{
    ...
    copter.user_nav->check_param_change();

    // pilot changes to circle rate and radius
    // skip if in radio failsafe
    if (!copter.failsafe.radio && copter.user_nav->pilot_control_enabled())
    {
        // update the circle controller's radius target based on pilot pitch stick inputs
        const float radius_current = copter.user_nav->get_radius();            // circle controller's radius target, which begins as the circle_radius parameter
        ...
        {
            copter.user_nav->set_radius_cm(radius_new);
        }
        ...
        if (g.radio_tuning != TUNING_CIRCLE_RATE)
        {
            ...
            {
                const float rate = copter.user_nav->get_rate();                 // circle controller's rate target, which begins as the circle_rate parameter
                const float rate_current = copter.user_nav->get_rate_current(); // current adjusted rate target, which is probably different from _rate
    ...
    copter.failsafe_terrain_set_status(copter.user_nav->update(target_climb_rate));
