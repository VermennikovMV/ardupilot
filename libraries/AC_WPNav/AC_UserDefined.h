...
class AC_UserDefined
{
public:
    /// Constructor
    AC_UserDefined(const AP_InertialNav &inav, const AP_AHRS_View &ahrs, AC_PosControl &pos_control);
    ...
    void set_center(const Vector3f &center, bool terrain_alt)
    {
        _center = center.topostype();
    ...
    void set_rangefinder_terrain_offset(bool use, bool healthy, float terrain_offset_cm)
    {
        _rangefinder_available = use;
        _rangefinder_healthy = healthy;
        _rangefinder_terrain_offset_cm = terrain_offset_cm;
    }
