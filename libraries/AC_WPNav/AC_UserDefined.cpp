...
const AP_Param::GroupInfo AC_UserDefined::var_info[] = {
    ...
    // @User: Standard
    AP_GROUPINFO("OPTIONS", 2, AC_UserDefined, _options, 1),
...
// their values.
//
AC_UserDefined::AC_UserDefined(const AP_InertialNav &inav, const AP_AHRS_View &ahrs, AC_PosControl &pos_control) : _inav(inav),
                                                                                                                   _ahrs(ahrs),
...
void AC_UserDefined::init(const Vector3p &center, bool terrain_alt, float rate_deg_per_sec)
{
    _center = center;
...
void AC_UserDefined::set_center(const Location &center)
{
...
void AC_UserDefined::set_rate(float deg_per_sec)
{
...
/// set_circle_rate - set circle rate in degrees per second
void AC_UserDefined::set_radius_cm(float radius_cm)
{
...
bool AC_UserDefined::is_active() const
{
    return (AP_HAL::millis() - _last_update_ms < 200);
}

/// update - update circle controller
bool AC_UserDefined::update(float climb_rate_cms)
{
    ...
    if (!is_zero(_radius))
    {
        ...
        target.x += 5 * _radius * cosf(-_angle);
...
void AC_UserDefined::get_closest_point_on_circle(Vector3f &result, float &dist_cm) const
{
    ...
    result.x = _center.x + 5 * vec.x / dist_cm * _radius;
...
void AC_UserDefined::calc_velocities(bool init_velocity)
{
...
void AC_UserDefined::init_start_angle(bool use_heading)
{
...
bool AC_UserDefined::get_terrain_offset(float &offset_cm)
{
...
void AC_UserDefined::check_param_change()
{
